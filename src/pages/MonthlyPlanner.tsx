    ...
    >
      {isEditing ? (
        <textarea
          ...
          className="w-full h-full min-h-[3.5rem] text-base md:text-xs text-slate-700 bg-white border border-slate-300 rounded p-1 focus:outline-none focus:ring-1 focus:ring-slate-500 resize-none"
          placeholder="目標..."
        />
      ) : (
        <div className="w-full h-full text-[10px] md:text-xs text-slate-700 break-words whitespace-normal cursor-text">
...
const TaskCell: React.FC<TaskCellProps> = ({ weekStart, tasks }) => {
  ...
  return (
    <div className="min-h-[4rem] border-b border-r border-slate-100 p-2 bg-slate-50/50 overflow-y-auto">
      <div className="flex flex-col gap-1">
        {weekTasks.map(task => (
          <div key={task.id} className="flex items-start gap-1 text-[9px] md:text-[10px] bg-white border border-slate-200 rounded px-1 py-0.5 md:px-1.5 md:py-1 shadow-sm">
            {task.isPriority && <AlertCircle className="w-2.5 h-2.5 md:w-3 h-3 text-rose-500 flex-shrink-0 mt-0.5" />}
            <span className={cn("break-words whitespace-normal", task.status === 'completed' && "line-through text-slate-400")}>
              {task.title}
            </span>
          </div>
        ))}
      ...
      <div className="flex-1 overflow-auto p-4 lg:p-10">
        ...
        <div className="min-w-[800px] bg-white border border-slate-200 rounded-3xl shadow-sm overflow-hidden">
          <div className="flex border-b border-slate-200">
            <div className="w-56 flex-shrink-0 bg-slate-50/50 p-4 flex items-end border-r border-slate-200">
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">教材 / 週</span>
            </div>
            {weeks.map((week, i) => (
              <div key={i} className="flex-1 bg-slate-50/50 p-4 text-center border-r border-slate-200 last:border-r-0">
                <span className="text-xs font-bold text-slate-600 tracking-tight">{getWeekLabel(week)}</span>
              </div>
            ))}
          </div>

          <div className="flex border-b border-slate-200">
            <div className="w-56 flex-shrink-0 p-4 flex items-center gap-2 bg-slate-50/30 border-r border-slate-200">
              <AlertCircle className="w-4 h-4 text-slate-400" />
              <span className="text-xs font-bold text-slate-500 uppercase tracking-wide">締め切り / 予定</span>
            </div>
            {weeks.map((week, i) => (
          ...
          {displayMaterials.map(material => (
            <div key={material.id} className="flex border-b border-slate-100 last:border-b-0 group">
              <div className="w-56 flex-shrink-0 p-4 flex items-center gap-3 bg-white border-r border-slate-200 group-hover:bg-slate-50 transition-colors">
                <div className={cn("w-3 h-3 rounded-full shadow-sm", material.color)} />
                <span className="text-sm font-bold text-slate-700 truncate" title={material.name}>
                  {material.name}
                </span>
              </div>
              {weeks.map((week, i) => (
