...
export default function Dashboard() {
  ...
  return (
    <div className="p-4 lg:p-10 max-w-6xl mx-auto w-full space-y-8">
      {/* Header & Bento Stats */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm flex items-center gap-5">
          ...
          <div>
            <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">連続学習記録</p>
            <p className="text-3xl font-black text-slate-900">{streak} <span className="text-sm font-bold text-slate-500">日</span></p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm flex items-center justify-between">
          <div className="space-y-1">
            <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">今日の進捗</p>
            <p className="text-2xl font-bold text-slate-900">
              {completedToday} / {totalToday} <span className="text-sm font-medium text-slate-500">完了</span>
            </p>
        ...
        <div className="md:col-span-2 bg-indigo-600 rounded-3xl p-6 text-white shadow-lg shadow-indigo-100 flex items-center gap-6 relative overflow-hidden">
          ...
          <div className="relative z-10">
            <h3 className="text-xs font-bold text-indigo-200 uppercase tracking-widest mb-1 flex items-center gap-2">
                  ...
                  return (
                    <div key={task.id} className={cn(
                      ...
                      isDone 
                        ...
                        : isOverdue 
                          ? "border-rose-200 bg-rose-50/30 hover:border-rose-300 hover:bg-white hover:shadow-xl hover:shadow-rose-50"
                          : "border-slate-100 bg-slate-50/30 hover:border-indigo-200 hover:bg-white hover:shadow-xl hover:shadow-indigo-50"
                    )}>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          {isOverdue && !isDone && (
                            <span className="text-[10px] bg-rose-600 text-white px-1.5 py-0.5 rounded-full font-bold uppercase tracking-tighter">最重要</span>
                          )}
                        </div>
                        {isOverdue && !isDone && (
                          <p className="text-[10px] font-bold text-rose-600 mt-1">期限切れ: {format(parseISO(task.deadline), 'M/d')}</p>
                        )}
                      </div>
                    </div>
                  );
        ...
        <div className="space-y-8">
                    ...
                    <div key={plan.id} className={cn(
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center justify-between mt-2">
                          ...
                          {isOverdue && !isDone && (
                            <p className="text-[10px] font-bold text-rose-600">期限切れ: {format(parseISO(plan.date), 'M/d')}</p>
                          )}
                        </div>
                      </div>
                    </div>
          ...
          <section className="bg-white border border-slate-200 rounded-3xl p-8 shadow-sm">
            ...
            <textarea
              ...
              className="w-full h-40 p-5 bg-slate-50 border border-slate-100 rounded-2xl text-base text-slate-700 placeholder:text-slate-400 focus:outline-none focus:ring-4 focus:ring-indigo-500/5 focus:border-indigo-500 transition-all resize-none font-medium leading-relaxed"
            />
          </section>
        </div>
      </div>
